...
The data set WESAD contains the physiological signal variables that are used in this study in addition to a number of engineered features. The selection of each variable was based on the fact that it has been previously used in autonomic nervous system studies and that it was relevant to the separation of neutral, stress, and amusement states.

*Electrocardiogram (ECG)* : ECG measures the electrical activity of the heart, from which heart rate (HR) and heart rate variability (HRV) are calculated. Because ECG is directly regulated by the sympathetic and parasympathetic nervous systems, it reliably reflects changes in cardiac rhythm during stress. We selected ECG as one of the two primary ARIMA targets because its beat-to-beat structure provides a naturally regular time series well suited to autoregressive modeling.

*Electrodermal Activity (EDA)* : EDA records the electrical conductivity of the skin, which rises with sympathetic-nervous-system-driven sweat gland activity. In our data, mean EDA under the Stress condition was roughly 40% higher than under Neutral. EDA was used as the second core ARIMA target — both as a standalone predictor and alongside ECG.

*Respiration (RESP)* : RESP captures the expansion and contraction of the chest during breathing. Faster, shallower, and more irregular breathing commonly accompanies stress, and in our data RESP variability was noticeably higher under Stress (std = 3.57) than at Neutral (std = 2.08). Because of this larger spread, RESP was treated as a secondary, corroborating signal rather than a primary ARIMA target.

*Skin Temperature (TEMP)* : TEMP reflects peripheral (not core) skin temperature, which tends to decrease under stress due to vasoconstriction. Of the four signals, TEMP showed the smallest separation across conditions, so it was used to enrich the Random Forest feature set rather than as a primary predictive target.

**Derived Variables**

*Label (target variable)* : Label (target variable): The categorical outcome variable Stess=1, Non-stress =0, as defined by the WESAD protocol (Schmidt and Reiss 2018). This is the variable both models ultimately predict — directly for Random Forest, and after converting a continuous forecast into a category for ARIMA.

*Subject* : A category identifier (S2, S3, S4, etc.) used to group observations by participant. It is not used as a predictor itself, but it is essential for preventing cross-subject data leakage: normalization, lag creation, and train/test splitting are all done independently within each subject's timeline so that no subject's history contaminates another's.

*Z-score / Standardized signal values* : Each raw signal (ECG, EDA, TEMP, RESP) was converted to a z-score using a StandardScaler fit only on that subject's training data. This corrects for the fact that physiological baselines vary widely between individuals (e.g., resting EDA may naturally be higher for one person than another), placing all subjects on a common scale before modeling.

*Lag features* : Four one-step-back values (e.g., EDA_lag_1 through EDA_lag_4) were added as new columns for each signal, giving the model roughly one second of history (at 4 Hz) at each row. These were included specifically to compensate for a structural weakness of Random Forest: unlike ARIMA, it has no built-in concept of time or sequence, so lag features manually inject short-term temporal context into an otherwise order-blind model.

*Rolling window features* : 1-minute and 3-minute rolling mean/std were computed for ECG and EDA. The 1-minute window is meant to capture short-term stress onset, while the 3-minute window captures sustained stress responses ([@ziyadidegan2025],[@oyeleye2022]). These features also give Random Forest a smoothed, denoised version of each signal, helping offset its sensitivity to momentary sensor noise or motion artifacts.
...
**Data Preparation and Signal Alignment Pipeline**
...
*Ingestion Protocols and Format Mapping* - The processing script programmatically interfaces with two pre-arranged disk files: wesad_wrist_train.csv (the training matrix) and wesad_wrist_test.csv (the holdout evaluation matrix). The ingestion sequence is mapped via the pandas framework using explicit memory allocation constraints:
    - Tabular Parsing: The continuous comma-separated streams are read into isolated memory frames (train_df and test_df) as multi-column tabular arrays.
    - DataFrame Casting: Arrays are strictly enforced as traditional mutable matrices (as.data frame structural equivalents) to allow downstream indexing and chronological row transformations.
    - Random State Initialization: To stabilize any internal stochastic data handling or 
    bootstrapping mechanisms during the initial load phase, a global seed is instantiated using random_state=42.
*Dynamic Channel Scanning and Feature Verification* - Because wearable sensor platforms can experience hardware dropouts, or variations between chest-worn and wrist-worn sensor matrices, the ingestion layer implements a dynamic channel scanner.
...
The ingestion layer dynamically scans for available sensor channels rather than assuming fixed column structure. Only signals present in the loaded CSV are passed downstream — if a sensor modality is missing, the pipeline skips it gracefully without crashing. This makes the pipeline robust to differences between chest and wrist sensor files.
...
### Physiological Signal Transformation (Our Solution):
To satisfy these assumptions and make the WESAD variables ready for prediction, we implemented three mandatory engineering transformations [@garg2021]:
...
standard deviation of 1. This scales the data safely into a uniform range across all     subjects without causing data leakage.
...
an 80/20 sequential split inside each isolated experimental condition block (Neutral, Stress,Amusement) [@schmidt2018].
...
### Justification of Model Selection for Physiological Stress Prediction
...
![Confusion Matrix for Stress states](RF_global_confusion_matrix.png){width=90%}


### Statistics table 

Based on our literature review, we selected ECG, EDA, TEMP and RESP as our primary physiological signals for stress detection. ECG captures heart rate and heart rate variability which consistently change during stress, as supported by ([@ziyadidegan2025],[@oyeleye2022]). EDA directly reflects sympathetic nervous system activity, the system activated during stress, while skin temperature complements it by capturing additional autonomic responses, both supported by [@sosa2026]. Respiration was included based on [@sosa2026] recommendation that combining EDA, RESP and TEMP provides a more complete picture of autonomic stress response than any single signal alone.
...
| Statistic | Value |
| :--- | :--- |
| Total observations (S2, S3, S4) | 4,508,701 |
| Number of subjects | 3 |
| Non-Stress observations (label 0) | 3,185,701 |
| Stress observations (label 1) | 1,323,000 |
| Missing values (any signal) | 0 |
| Sampling rate | 700 Hz |
| Signals used | ECG, EDA, TEMP, RESP |


### Wrist Data Exploration

| Statistic | Value |
| :--- | :--- |
| Total observations (S2, S3, S4) | 25,764 |
| Number of subjects | 3 |
| Non-Stress observations (label 0) | 18,204 |
| Stress observations (label 1) | 7,560 |
| Missing values (any signal) | 0 |
| Sampling rate | 4 Hz |
| Signals used | EDA, TEMP |
...
| Subject | Precision | Recall | F1-Score | Accuracy |
|---------|-----------|--------|----------|----------|
| S2      | 1.00      | 1.00   | 1.00     | 1.00     |
| S3      | 0.95      | 0.92   | 0.94     | 0.95     |
...
| Subject | Top Feature | Importance | 2nd Feature | Importance |
|---------|-------------|------------|-------------|------------|
| S2      | TEMP_roll3min_mean | 0.218 | RESP_roll3min_std | 0.146 |
| S3      | ECG_roll1min_std | 0.269 | RESP_roll1min_std | 0.103 |
| S4      | EDA | 0.217 | EDA_roll1min_mean | 0.140 |

*Aggregate feature importance:*

| Feature | Importance |
|---------|------------|
| ECG_roll1min_std | 0.0997 |
| EDA | 0.0930 |
| TEMP_roll3min_mean | 0.0852 |
| RESP_roll3min_std | 0.0663 |
| TEMP_lag_1 | 0.0536 |

## ROC-AUC Random Forest

  ![Chest and Wrist ROC AUC curves](rf_roc_curves_combined.png){width=90%}

### Conclusion for Random Forest

Random Forest achieved strong stress state classification performance 
on the WESAD dataset, with subjects S2 and S4 reaching perfect 
classification (F1 = 1.00) using chest sensor signals, while subject 
S3 achieved an F1 score of 0.94 reflecting physiological variability 
in stress response patterns across individuals. Chest sensors 
(ECG, EDA, TEMP, RESP at 700Hz) consistently outperformed wrist 
sensors (EDA, TEMP at 4Hz), with subject S2 showing notably lower 
wrist performance (F1 = 0.47) compared to perfect chest performance, 
confirming that signal richness and sampling rate directly impact 
classification accuracy. Feature importance analysis revealed that 
the dominant predictive signal varied by subject where TEMP rolling 
features drove S2 classification, ECG variability drove S3, and EDA 
drove S4, suggesting that stress manifests differently across 
individuals and that personalized per subject modeling is more 
appropriate than a single generalized classifier. A direct comparison 
between Random Forest and ARIMA performance will be discussed once 
ARIMA results are available, with ARIMA achieving [Virginia to add 
results here] on the same signals.
