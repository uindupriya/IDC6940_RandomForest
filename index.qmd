...
Overall the Random Forest model performed better than the ARIMA and classfication for most signals and participants, with some interesting behaviors that emerged due to the nature of each physiological signal.
...
As expected, the ARIMA did not perform as well as Random Forest when predicting ECG, even with the improved prediction from longer rolling windows. The example above shows how signals with a complex temporal patterns are harder to predict for a univariate forecasting model like ARIMA. This resulted in the predicted values not having a meaniful separation between baseline and stress state for accrurate classification using nearest-means.

 As observed by the signals with less complexity, the ARIMA-based classification performes better with a more distince difference in baseline and stress state. A useful metric to forecast with in next steps would be HR and HRV, instead of raw ECG. HR and HRV typically show more obvious physiological changes when in stess state, and classification rules using nearest mean may be more meaningful for detecting stress. 
