...
Random Forest classifiers were trained separately for each subject (S2, S3, S4) and signal (ECG, EDA, RESP, TEMP), using an ensemble of B = 100 decision trees on rolling window features (mean, standard deviation, minimum, and maximum) engineered from the normalized signals at 1-minute and 3-minute intervals. Random Forest achieved a mean accuracy of 0.76 and mean F1 of 0.74 across all signals and subjects at the 1-minute window, improving modestly to a mean accuracy of 0.78 and mean F1 of 0.76 at the 3-minute window with a gain of roughly two points in both metrics. Performance was strongest and most consistent for TEMP and ECG, with 3-minute TEMP accuracy reaching 0.95 for both S3 and S4, and 3-minute ECG accuracy reaching 0.96 for S3. RESP classification was moderate and stable across subjects, with accuracy consistently in the 0.75–0.78 range at both window sizes. EDA showed by far the widest subject-to-subject swing of any signal: Random Forest achieved perfect classification for S4 (1.0000 accuracy and F1 at both windows) but near-zero performance for S2 (0.0257 accuracy at 1-minute, 0.0389 at 3-minute), indicating that EDA's predictive value was highly subject-dependent rather than reflecting a stable underlying stress signal.

#### Rolling windows for RandomForest

| Signal | Subject | RF Accuracy | RF Precision | RF Recall | RF F1 |
| :----: | :-----: | ----------: | ------------: | --------: | -----: |
|   ECG  |    S2   |      0.7890 |        0.7703 |    0.7569 | 0.7624 |
|   ECG  |    S3   |      0.8920 |        0.9036 |    0.8620 | 0.8770 |
|   ECG  |    S4   |      0.8359 |        0.8232 |    0.8138 | 0.8180 |
|   EDA  |    S2   |      0.0257 |        0.0300 |    0.0262 | 0.0257 |
|   EDA  |    S3   |      0.7562 |        0.8622 |    0.6609 | 0.6635 |
|   EDA  |    S4   |      1.0000 |        1.0000 |    1.0000 | 1.0000 |
|  RESP  |    S2   |      0.7325 |        0.7066 |    0.6812 | 0.6886 |
|  RESP  |    S3   |      0.7746 |        0.7861 |    0.7124 | 0.7259 |
|  RESP  |    S4   |      0.7413 |        0.7197 |    0.6935 | 0.7013 |
|  TEMP  |    S2   |      0.6936 |        0.7665 |    0.7644 | 0.6936 |
|  TEMP  |    S3   |      0.9521 |        0.9652 |    0.9334 | 0.9463 |
|  TEMP  |    S4   |      0.9549 |        0.9435 |    0.9651 | 0.9520 |

*Table 1: Random Forest chest sensor classification results using rolling window features at 1min intervals (ECG, EDA, TEMP, RESP at 700Hz).*


| Signal | Subject | RF Accuracy | RF Precision | RF Recall | RF F1 |
| :----: | :-----: | ----------: | ------------: | --------: | -----: |
|   ECG  |    S2   |      0.8429 |        0.8282 |    0.8249 | 0.8265 |
|   ECG  |    S3   |      0.9587 |        0.9625 |    0.9479 | 0.9545 |
|   ECG  |    S4   |      0.9101 |        0.8970 |    0.9139 | 0.9039 |
|   EDA  |    S2   |      0.0389 |        0.0387 |    0.0470 | 0.0387 |
|   EDA  |    S3   |      0.7784 |        0.8707 |    0.6920 | 0.7038 |
|   EDA  |    S4   |      1.0000 |        1.0000 |    1.0000 | 1.0000 |
|  RESP  |    S2   |      0.7470 |        0.7233 |    0.7023 | 0.7094 |
|  RESP  |    S3   |      0.7835 |        0.7935 |    0.7252 | 0.7394 |
|  RESP  |    S4   |      0.7560 |        0.7375 |    0.7104 | 0.7188 |
|  TEMP  |    S2   |      0.6800 |        0.7611 |    0.7539 | 0.6799 |
|  TEMP  |    S3   |      0.9509 |        0.9644 |    0.9317 | 0.9449 |
|  TEMP  |    S4   |      0.9485 |        0.9364 |    0.9601 | 0.9453 |

*Table 2: Random Forest chest sensor classification results using rolling window features at 3min intervals (ECG, EDA, TEMP, RESP at 700Hz).*
...
*Table 3: ARIMA error metrics for 1 min rolling windows.*
...
*Table 4: ARIMA error metrics for 3 min rolling windows.*
...
*Table 5: Random Forest and ARIMA chest sensor classification results using 
...
*Table 6: Random Forest and ARIMA chest sensor classification results using 
rolling window features at 3min intervals (ECG, EDA, TEMP, RESP at 700Hz).*

### Model Comparison Figures (3-minute Window)

Overall the Random Forest model performed better than the ARIMA and classification for most signals and participants, with some interesting behaviors that emerged due to the nature of each physiological signal.
...
As expected, the ARIMA did not perform as well as Random Forest when predicting ECG, even with the improved prediction from longer rolling windows. The example above shows how signals with a complex temporal patterns are harder to predict for a univariate forecasting model like ARIMA. This resulted in the predicted values not having a meaningful separation between baseline and stress state for accurate classification using nearest-means.

 As observed by the signals with less complexity, the ARIMA-based classification performes better with a more distince difference in baseline and stress state. A useful metric to forecast with in next steps would be HR and HRV, instead of raw ECG. HR and HRV typically show more obvious physiological changes when in stress state, and classification rules using nearest mean may be more meaningful for detecting stress. 
...
In the case above, both models performed well, showing how metrics with a significant physiological difference between baseline and stress state allows for even a simple heuristic model based on nearest-mean classification can achieve accurate classification. This suggests that when there is a clear physiological difference when under stress, complex machine learning algorithms may provide only small improvements. 
...
In this case, RESP proved to be a more difficult classification problem. Although neither model had near-perfect classification, we see again how Random Forest has a greater ability to capture patterns related to stress and provided better results than the ARIMA-based approach. This difference in performance was consistently shown when there was a less obvious physiological difference in stress state, limiting the effectiveness of mean-based classification.
...
This final case is shows ARIMA's unique behavior with perfect recall for stress, but a much lower precision due to many false positives. This is seen not only in Temperature, but other signals where the nearest-mean classification tends to favor stress class when the prediction fell between baseline and stress state. The result is an increase in false alarms, reducing overall classification performance. Random Forest consistently produced a more balanced classifier by leveraging multiple rolling-window features (mean, standard deviation, minimum, and maximum), allowing it to better capture temporal differences in stress state and resulting in higher overall accuracy and F1 scores.

## Conclusion

**Results:** This paper performed a comparative analysis of two classifiers in two different data window sizes (1-minute, 3-minute) for all three subjects (S2–S4) of stress states derived from chest sensor data (ECG, EDA, TEMP, RESP) using ARIMA and Random Forest classifiers. Random Forest consistently outperformed ARIMA overall — mean accuracy was 0.76 vs. 0.53 (1-min) and 0.78 vs. 0.54 (3-min), with an even wider F1 gap (0.74 vs. 0.31 and 0.76 vs. 0.43). There was a wide range of this gap across signals. However, ARIMA fared poorly in the ECG and RESP datasets, and its ROC curves yielded AUC near or below 0.5 (random), indicating that the ARIMA-based nearest-mean classification is not valid for these signals when capturing their stress-sensitive behavior. The only time that ARIMA outperformed Random Forest was during the 3-minute window for S2 (Random Forest F1 was 0.6799, while ARIMA's was 0.6840). The most erratic signal was EDA, a signal that went from near-perfect (S4) to near-zero (S2), indicating that subject-specific signal characteristics likely had substantial influence on classification performance. Increasing the window size from 1 minute to 3 minutes showed limited improvement in both model, but helped ARIMA-based classification, with an average F1 improved on the order of 13 points with the larger 3 minute window. This corroborates the findings of ARIMA's improved ECG forecasting accuracy at longer windows, which in turn helps nearest-mean classification.

**Implications:** This does not confirm the potential of ARIMA-based classification in every situation, as for the ECG and respiration signals, the added cost of Random Forest is clearly warranted because they are fast and complex signals. However, for more slowly varying signals, such as temperature, ARIMA achieved comparable performance that, in real-time, resource-constrained wearable situations, might be the more practical option because it is simpler and less computation intensive. The choice of model is not necessarily one-size-fits-all, but rather model choice may be more appropriately signal-dependent. Because of the small sample size (3 out of 15 subjects available in WESAD) and the simplicity of the ARIMA-based nearest-means classification, which often seemed biased to the majority class, these results are to be interpreted with some care. Future studies should investigate more sophisticated decision thresholds or probabilistic decision strategies that incorporate ARIMA prediction uncertainty and better utilize temporal dynamics, to potentially improve classification performance relative to Random Forest.